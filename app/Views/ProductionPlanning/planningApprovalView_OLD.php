                ...
                <tbody id="reorderBody">
                    <?php foreach ($pendingReorders as $rp): ?>
                        <?php
                            ...
                            $orig = reset($orig);
                        ?>
                        <tr data-id="<?= esc($rp['PLANNING_CAL_ID']) ?>" data-machine="<?= esc($rp['MACHINE']) ?>">
...
$(function () {
    ...
    $("#approveAll").on("click", function () {
        ...
        $("#reorderBody tr:visible").each(function () {
            ...
            if (id) planning_cal_ids.push(id);
        });

        console.log(planning_cal_ids);

        if (planning_cal_ids.length === 0) {
